...
- **✨ Gợi ý bằng AI**: nút trong hàng 2 cột gọi AI adapter cục bộ (`http://127.0.0.1:4924`) để sinh nội dung 2 trường `Direct Cause of Defect` và `Correction Action` (tiếng Việt, ngắn gọn) dựa trên Summary/Description của bug + Resolution/Defect Origin/Defect Type/Cause Category đang chọn. Bấm là **ghi đè** 2 trường. **Yêu cầu**: adapter phải đang chạy và đã đăng nhập (extension tự dò qua `/health`; nếu lỗi sẽ báo ngay dưới nút). Vì adapter không có CORS, mọi request đi qua **background service worker** (`background.js`) — cần `host_permissions` cho `http://127.0.0.1:4924/*` (đã khai trong `manifest.json`).
...
| File | Vai trò |
|------|---------|
| `manifest.json` | Manifest V3: content script, popup, background SW, icons, quyền `storage` + `host_permissions` adapter |
| `content.js` | Logic ẩn trường, nút toggle, re-sync động, auto-fill, bố cục 2 cột, nút AI, bật/tắt |
| `content.css` | Class ẩn trường + style nút toggle, bố cục 2 cột, thanh nút AI |
| `background.js` | Service worker: transport gọi AI adapter (fetch, timeout, map lỗi) — tránh CORS |
